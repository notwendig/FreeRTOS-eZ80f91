...
/*
	...
	File: 	ez80_rtc.c
			module rtc driver.
			Part of FreeRTaOS Port for the eZ80F91 Development Kit eZ80F910300ZCOG
...
const char *const mon[12]= {"Jan","Feb","Mar","Apr","Mai","Jun","Jul","Aug","Sep","Oct","Nov","Dec"};

#define RTCREG(x) (*(volatile unsigned char __INTIO *)(0xE0 + (x)))

#define k1 1461UL	// (365 * 4 + 1)
...
time_t date_time_to_epoch( rtc_t* date_time)
{
    ...
    return (time_t) ((((year/4UL*k1+days[year%4][month]+day)*24+hour)*60+minute)*60+second);
}
...
void epoch_to_date_time(rtc_t* date_time, time_t epoch)
{
    ...
    date_time->d.buff[RID_SEC] = epoch % 60UL; 
	epoch /= 60UL;
    date_time->d.buff[RID_MIN] = epoch % 60UL; 
	epoch /= 60UL;
    date_time->d.buff[RID_HRS] = epoch % 24UL; 
	epoch /= 24UL;
	date_time->d.buff[RID_DOW] = ((epoch +4) % 7);
...
char *getsAlarm(char *s, size_t sz)
{
	...
	getRTC(&p);
	snprintf(s, sz, "%s. %2.2hu:%2.2hu:%2.2hu [%c%c%c%c%c]",dow[p.d.r.ADOW-1], p.d.r.AHRS, p.d.r.AMIN, p.d.r.ASEC
...
char *getsTime(char *s, size_t sz)
{
	...
	p.flg = (1 << RID_HRS) | (1 << RID_MIN) | (1 << RID_SEC); 
	getRTC(&p);
	snprintf(s, sz, "%2.2hu:%2.2hu:%2.2hu", p.d.r.HRS, p.d.r.MIN, p.d.r.SEC);
	return s;
}
...
char *getsDate(char *s, size_t sz)
{
	...
	if(chkRTCPowerLost())
		...
		snprintf(s, sz,"%s. %2.2hu %s. %4.4hu", dow[p.d.r.DOW-1], p.d.r.DOM, mon[p.d.r.MON-1], p.d.r.CEN*100+p.d.r.YR+1970);
	return s;
}
